...
🔢 Compared **${pluralize(diffToPrevious.changes.length, "schema change")}** against **${pluralize(diffToPrevious.numberOfCheckedOperations, "operation")}** seen over the **last ${formatTimePeriod(hours)}**.
${breakingChanges.length > 0
        ? `❌ Found **${pluralize(diffToPrevious.changes.filter(change => change.type === "FAILURE")
            .length, "breaking change")}** that would affect **${pluralize(affectedQueryCount, "operation")}** across **${pluralize(diffToPrevious.affectedClients && diffToPrevious.affectedClients.length, "client")}**`
                ...
                return [
                    {
                        ...
                        task: async (ctx, task) => {
                            ...
                            const newContext = {
                                checkSchemaResult: await project.engine.checkSchema(Object.assign({ id: configName, schema: graphql_1.introspectionFromSchema(schema).__schema, tag: flags.tag, gitContext: await git_1.gitInfo(this.log), frontend: flags.frontend || config.engine.frontend }, (historicParameters && { historicParameters }))),
                                config,
                        ...
                        task: async (ctx, task) => {
                            ...
                            const hours = validationConfig
                                ? Math.abs(moment_1.default()
                                    ...
                                    .diff(moment_1.default().add(validationConfig.to, "second"), "hours"))
                                : null;
                            task.title = `Compared ${pluralize(chalk_1.default.blue(schemaChanges.length.toString()), "schema change")} against ${pluralize(chalk_1.default.blue(numberOfCheckedOperations.toString()), "operation")}${hours
                    ...
                    },
                    {
                        ...
                        task: async (ctx, task) => {
                            ...
                            task.title = `Found ${pluralize(chalk_1.default.blue(breakingSchemaChangeCount.toString()), "breaking change")} and ${pluralize(chalk_1.default.blue(nonBreakingSchemaChangeCount.toString()), "compatible change")}`;
